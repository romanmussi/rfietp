<?php
/**
 * Offers fixtures for web tests.
 *
 * Just put this file into your libs folder and import it into your web test
 * files. Then extend your web test files from CakeWebTestCaseWithFixtures
 * instead of CakeWebTestCase. Then define what fixtures you want to be
 * loaded before each test using the $fixtures variable. The whole thing
 * should look something like the following:
 *
 * <?php
 * App::import('Lib', 'CakeWebTestCaseWithFixtures');
 *
 * class MyWebTestCase extends CakeWebTestCaseWithFixtures {
 *   var $fixtures = array('app.user', 'app.post');
 *
 *   function testTrue() {
 *     $this->assertTrue(true); // Yaayyy!!!
 *   }
 * }
 * ?>
 *
 * Be sure to specify all the fixtures that are needed by the part of your
 * application being tested (e.g. app.user, app.role, app.aco, app.aro,
 * app.aros_acos etc.)!
 *
 * CakeWebTestCaseWithFixtures makes use of the startTest() method, so
 * remember to call parent::startTest() if you override it!
 *
 * For more information see http://josh.ch/wordpress/?p=8
 */
 
class CakeWebTestCaseWithFixtures extends CakeWebTestCaseWithDbFunctionality {
  /**
   * This is the starting point where the scripted browser will be after the
   * start of every test.
   *
   * @var array
   * @access public
  ...
  /**
   * This method is called automatically before a test is run and puts
   * everything into place so CakePHP will connect to the test database.
   *
   * Please remember to call parent::startTest() if you override this method
   * in your tests!
   */
  function startTest($method) {
    $this->identifyAsWebtest();
    parent::startTest($method);
  }
  
  /**
   * Puts everything into place so CakePHP will connect to the test database.
   *
   * This is done by adding a User-Agent header "CakePHP/Webtest" which is
   * passed to the website with every request. When this header is submitted,
...
class CakeWebTestCaseWithDbFunctionality extends CakeWebTestCase {
  ...
  // END OF CODE COPY&PASTING FROM cake/tests/lib/cake_test_case.php
  //////////////////////////////////////////////////////////////////
}
?>